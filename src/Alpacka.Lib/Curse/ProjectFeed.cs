...
namespace Alpacka.Lib.Curse
{
    public class ProjectFeed
    {
        ...
        private static readonly string MODS_URL = "https://cursemeta.nikky.moe/mods.json"; 
        private static readonly string MODPACKS_URL = "https://cursemeta.nikky.moe/modpacks.json"; 
        // private static readonly string COMPLETE_URL = "https://cursemeta.nikky.moe/complete.json.bz2"; 
        
        public static async Task<ProjectList> GetMods()
        {
            //var cache = Path.Combine(Constants.CachePath, "curse");
            //Debug.WriteLine($"cache: { cache }");
            
            using (var fileCache = new FileCache(Path.Combine(Constants.CachePath, "curse", "feed")))
            using (var downloader = new FileDownloader(fileCache))
            {
                ...
                var feed = await downloader.Download(MODS_URL);
                
                //var completeFile = Path.Combine(cache, "mods.json");
        ...
        public static async Task<ProjectList> GetModPacks()
        {
            //ar cache = Path.Combine(Constants.CachePath, "curse");
            //Debug.WriteLine($"cache: { cache }");
            ...
            {
                Console.WriteLine("Downloading cursemeta project data. This could take a while.");
                var feed = await downloader.Download(MODPACKS_URL);
                
                //var completeFile = Path.Combine(cache, "modpacks.json");
